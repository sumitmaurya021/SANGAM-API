...
RSpec.describe "Posts API", type: :request do
  ...
  describe 'POST /api/v1/posts(.:format)' do
    ...
    it 'executes the request and returns a valid status' do
      ...
      post "/api/v1/posts", params: { post: valid_attributes }, headers: headers
  ...
  describe 'PATCH /api/v1/posts/:id(.:format)' do
    ...
    it 'executes the request and returns a valid status' do
      valid_attributes = attributes_for(:post) rescue {}
      patch "/api/v1/posts/#{post.id}", params: { post: valid_attributes }, headers: headers
      expect(response.status).to be_between(200, 422)
  ...
  describe 'PUT /api/v1/posts/:id(.:format)' do
    ...
    it 'executes the request and returns a valid status' do
      valid_attributes = attributes_for(:post) rescue {}
      put "/api/v1/posts/#{post.id}", params: { post: valid_attributes }, headers: headers
      expect(response.status).to be_between(200, 422)
