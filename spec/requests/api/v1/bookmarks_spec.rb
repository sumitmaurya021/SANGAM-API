...
RSpec.describe "Bookmarks API", type: :request do
  ...
  describe 'POST /api/v1/bookmarks(.:format)' do
    ...
    it 'executes the request and returns a valid status' do
      ...
      post "/api/v1/bookmarks", params: { bookmark: valid_attributes }, headers: headers
  ...
  describe 'PATCH /api/v1/bookmarks/:id(.:format)' do
    ...
    it 'executes the request and returns a valid status' do
      valid_attributes = attributes_for(:bookmark) rescue {}
      patch "/api/v1/bookmarks/#{bookmark.id}", params: { bookmark: valid_attributes }, headers: headers
      expect(response.status).to be_between(200, 422)
  ...
  describe 'PUT /api/v1/bookmarks/:id(.:format)' do
    ...
    it 'executes the request and returns a valid status' do
      valid_attributes = attributes_for(:bookmark) rescue {}
      put "/api/v1/bookmarks/#{bookmark.id}", params: { bookmark: valid_attributes }, headers: headers
      expect(response.status).to be_between(200, 422)
